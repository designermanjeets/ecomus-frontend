...
export class OrderState {
  ...
  placeOrder(ctx: StateContext<OrderStateModel>, action: PlaceOrder) {
    return this.orderService.placeOrder(action?.payload).pipe(
      tap({
        next: result => {
          if((action.payload.payment_method == 'cod' || action.payload.payment_method == 'cash_free' || action.payload.payment_method == 'sub_paisa' || action.payload.payment_method == 'bank_transfer') && !result.is_guest) {
            this.router.navigateByUrl(`/account/order/details/${result.order_number}`);
            setTimeout(() => { window.location.reload() }, 1000);
          } else if((action.payload.payment_method == 'cod' || action.payload.payment_method == 'cash_free' || action.payload.payment_method == 'sub_paisa' || action.payload.payment_method == 'bank_transfer') && result.is_guest) {
